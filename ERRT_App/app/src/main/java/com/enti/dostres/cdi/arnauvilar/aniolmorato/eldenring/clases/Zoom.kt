package com.enti.dostres.cdi.arnauvilar.aniolmorato.eldenring.clases
...
class Zoom : AppCompatImageView, View.OnTouchListener,
    ...
    fun fixTranslation() {
        mMatrix!!.getValues(mMatrixValues) // Pone los valores de matrix en un array de floats para utilizarlos luego
        val transX = mMatrixValues!![Matrix.MTRANS_X] // Get de la transición en X más reciente
        val transY = mMatrixValues!![Matrix.MTRANS_Y] // Get de la transición en Y más reciente
    ...
    private fun getFixTranslation(trans: Float, viewSize: Float, contentSize: Float): Float {
        val minTrans: Float
        val maxTrans: Float
        if (contentSize <= viewSize) { // Caso: NOT ZOOMED
            minTrans = 0f
        ...
        if (trans < minTrans) { // Translación x o y negativos (abajo o derecha)
            return -trans + minTrans
        }
        if (trans > maxTrans) { // Translación x o y positivos (arriba o izquierda)
